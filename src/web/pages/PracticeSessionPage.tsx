...
export function PracticeSessionPage({ sessionId, questions, recordAttempt, loadReview, onExit, onBack, onMastered, initialProgress, onProgressChange = () => undefined }: Props) {
  ...
  return <section className="page question-page"><button className="back-button" onClick={onBack} aria-label="返回練習選擇"><ArrowLeft aria-hidden="true" />返回練習選擇</button><div className="progress-label"><span>第 {index + 1} 題，共 {questions.length} 題</span><span>{question.subject} · {question.chapter}</span></div><div className="progress"><span style={{ width: `${((index + 1) / questions.length) * 100}%` }} /></div><div className="question-heading"><span className="step-kicker">現在專心這一題</span><h2>{question.questionText}</h2></div><div className="options">{question.options.map((option) => <button key={option.id} className={optionClass(option.id, selected, review)} onClick={() => void answer(option.id)} disabled={Boolean(selected)}><span className="radio" />{option.sourceLabel && <span className="option-label">{option.sourceLabel}</span>}<span>{option.text}</span></button>)}</div>{!review && <button className="primary answer-button" onClick={() => void viewAnswer()}>公布答案</button>}{review && <section className="answer-panel"><strong>{resultLabel}</strong><section className="explanation-block source-explanation"><h3>原始解析</h3><p>{sourceExplanation}</p></section>{review.aiExplanation.content && <section className="explanation-block ai-explanation"><h3>AI 白話解析</h3><p>{review.aiExplanation.content}</p>{review.aiOptionAnalysis && review.aiOptionAnalysis.length > 0 && <div>{review.aiOptionAnalysis.map((analysis, analysisIndex) => <div className="analysis-row" key={analysis.optionId}><strong>{question.options[analysisIndex]?.sourceLabel ?? `選項 ${analysisIndex + 1}`}：{analysis.verdict === "correct" ? "正確" : analysis.verdict === "incorrect" ? "錯誤" : "待確認"}</strong><p>{analysis.content}</p></div>)}</div>}</section>}<div className="inline-actions"><button onClick={() => void onMastered(question.id)}>標記已掌握</button><button className="primary" onClick={next}>{index + 1 === questions.length ? "完成練習" : "下一題"}</button></div></section>}{message && <p className="notice">{message}</p>}</section>;
}
